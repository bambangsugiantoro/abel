...
// Cari URL pembayaran dari response SumoPod
function getCheckoutUrl(response: any): string {
  if (!response) return '';
  let url = '';
  const search = (item: any) => {
    if (!item) return;
    if (typeof item === 'string' && item.startsWith('http') && (item.includes('checkout') || item.includes('pymnt') || item.includes('pay') || item.includes('invoice'))) {
      if (!url) url = item;
...
export default function HalamanPembayaran() {
  ...
  const handleBuatQRIS = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (checkoutUrl) {
        // Otomatis arahkan siswa langsung ke halaman QRIS resmi SumoPod yang valid
        ...
        <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-gray-200">
          ...
          <form onSubmit={handleBuatQRIS} className="space-y-4">
            ...
            >
              {loading
                ? 'Membuka QRIS Resmi...'
                : `Lanjut Bayar Rp ${(paketPilihan?.price || 0).toLocaleString('id-ID')} via QRIS`}
            </button>
          </form>
        </div>
